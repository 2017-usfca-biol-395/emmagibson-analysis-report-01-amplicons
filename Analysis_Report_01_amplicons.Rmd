...
```{r species-diversity}
# plots species diversity in male vs female hands
agg <- aggregate(sequence_table_nochim,
          by = list(sex = metadata_in$sex_s),
          FUN = sum)

row.names(agg) <- agg$sex
...
The first graph I made shows rarefaction curvesfor male hand, female hand, and mouse/keyboard samples.

**psmelt
females habve more diversity, males have more samples, both are an adequate representation of diversity
-plot richness?
...
```
Fig : Alpha diversity measures of the two sample types, colored by gender.
...
ggplot(melted_phyloseq_obj,
  ...
  ggtitle("Species abundance in hands and mice") +
